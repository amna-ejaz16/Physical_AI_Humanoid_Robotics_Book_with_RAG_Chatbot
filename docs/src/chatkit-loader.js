/**
 * ChatKit Widget Loader for Docusaurus
 * This module is loaded on every page via clientModules
 */

// Import CSS styles from src/css (Webpack-compatible path)
import './css/chatkit-styles.css';

// Configuration
const CHATKIT_CONFIG = {
  // Backend API endpoint (change for production)
  backendUrl: typeof window !== 'undefined' && window.CHATKIT_BACKEND_URL
    ? window.CHATKIT_BACKEND_URL
...
function initializeChatKit() {
  ...
  if (window.chatkitInitialized) {
    console.log('[ChatKit] Already initialized, skipping');
    return;
  }
  ...
  import('https://cdn.platform.openai.com/deployments/chatkit/chatkit.js')
    .then(() => {
      ...
      // Mark as initialized
      window.chatkitInitialized = true;
